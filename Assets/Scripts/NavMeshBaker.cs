...
public class NavMeshBaker : MonoBehaviour
{
    ...
    private IEnumerator CheckPlayerMovement()
    {
        ...
        while (true)
        {
            if (Vector3.Distance(WorldAnchor, Yeti.transform.position) > MovementThreshold)
            {
                BuildNavMesh(true);
                WorldAnchor = Yeti.transform.position;
            }
    ...
    {
        Bounds navMeshBounds = new Bounds(Yeti.transform.position, NavMeshSize);
        ...
        if (Async)
        {
            NavMeshBuilder.UpdateNavMeshDataAsync(NavMeshData, Surface.GetBuildSettings(), Sources, new Bounds(Yeti.transform.position, NavMeshSize));
        }
        else
        {
            NavMeshBuilder.UpdateNavMeshData(NavMeshData, Surface.GetBuildSettings(), Sources, new Bounds(Yeti.transform.position, NavMeshSize));
        }
    }
}